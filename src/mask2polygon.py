...
def initial_line_extraction(label_mask,label_num):
    ...
    for i in range(1,label_num+1):
        ...
        dp_pt=rdp(contours[0],epsilon=2, algo="iter", return_mask=False)
        #dp_pt=rdp(contours[0], epsilon=7, algo="iter", return_mask=False)
        temp_line=[]
...
def line_adjustment(dp_line, label_num):
    ...
    for i in range(label_num):
        ...
        for j, line_p in enumerate(dp_line[i]):
            # temp_line=np.array(line_p)  # numpy==1.22.4
            ...
            #temp_line=np.array(line_p)  # numpy==1.22.4
            line_loc=np.array([(line_p[1][0]+line_p[2][0])/2,\
                               (line_p[1][1]+line_p[2][1])/2])
            line_len=sqrt((line_p[2][0]-line_p[1][0])**2\
                             +(line_p[2][1]-line_p[1][1])**2)
            poly1_line.append([line_p[0],line_loc,line_len,line_p[1][:],line_p[2][:],line_p[3]])
        poly2_line=[]
        flag=1
        ...
        while (len(poly2_line)<len(dp_line[i])):
            ...
            temp_line_P=list(poly1_line[k] for k in list(angle_idxt1))
            temp_line_V=list(poly1_line[k] for k in list(angle_idxt2))
...
def line_regularization(adj_line, label_num, line_reg_thr, img_ortho_t):
    ...
    for i in range(label_num):
        ...
        max_angle_idx=np.argmax(np.array([k[6] for k in reg_line_final]))
...
def run_fit_main(image, out_name, Tl):
    from scipy import ndimage
    import os
    import json
    #image = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)  # IMREAD_UNCHANGED ensures we preserve the original image depth and channels
    # exclude small region in building mask
    img_mask = np.zeros((image.shape[0], image.shape[1]), dtype=np.uint8)
    if np.max(image) == 255:
        img_mask[np.where(image > 5)] = 1
    else:
        img_mask = image
    bw_mask = morphology.remove_small_objects(img_mask.astype(bool), min_size=200, connectivity=1)
    cv2.imwrite(os.path.join(out_name,"mask_removal.png"),bw_mask*255)
    bw_mask = ndimage.binary_fill_holes(bw_mask).astype(bool)
    cv2.imwrite(os.path.join(out_name,"mask_removal_filled.png"),bw_mask*255)
    #img_ortho_t = cv2.imread(mask_path, cv2.IMREAD_UNCHANGED)
    img_ortho_t=copy.deepcopy(image)
    if len(img_ortho_t.shape)==2:
        img_ortho_t=cv2.merge([img_ortho_t,img_ortho_t,img_ortho_t])
    # connectivity detection
    L_mask, L_num = measure.label(bw_mask, return_num=True, connectivity=2)
    cv2.imwrite(os.path.join(out_name,"mask_connect.png"),L_mask)
    ...
    img_ortho_bound_init = copy.deepcopy(img_ortho_t)
    lines_init=[]
    for line_group in ini_line:
        for line_dr in line_group:
            x0 = int(round(line_dr[1][0]))
            y0 = int(round(line_dr[1][1]))
            x1 = int(round(line_dr[2][0]))
            y1 = int(round(line_dr[2][1]))
            cv2.line(img_ortho_bound_init, (x0, y0), (x1, y1), 255, 1, cv2.LINE_AA)
            lines_init.append([x0,y0,x1,y1])
    lines_init=np.array(lines_init)
    lines_init=np.expand_dims(lines_init,axis=1)
    cv2.imwrite(os.path.join(out_name, 'boundary_init.png'), img_ortho_bound_init)
    # 2. line adjustment
...
if __name__ == '__main__':
    '''
    given geo-referenced image and mask
    generate geo-referenced shp file
    '''

    mask_path = r'E:\test\sat\class.png'
    out_folder = r'E:\test\sat'
    os.makedirs(out_folder, exist_ok=True)


    # # binary to polygon. linear regularization
    Tl = 90
    image = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
    lines=run_fit_main(image, out_folder, Tl)
    a=1
